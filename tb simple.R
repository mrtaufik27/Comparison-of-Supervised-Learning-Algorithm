...
glm(family=binomial,data=m,tb~.) -> mod
summary(mod)
drop1(mod,test="Chisq")

# the ghos, variable mentions the locations of death, is not significant, however drop1 result indicates this variable is important, then we consider to combine it with DRgrp varible
...
pval1 <- ifelse(pval[1]<0.05,"<0.05",round(pval[1],5))
pval2 <- ifelse(pval[2]<0.05,"<0.05",round(pval[2],5))
pval3 <- ifelse(pval[3]<0.05,"<0.05",round(pval[3],5))
pval4 <- ifelse(pval[4]<0.05,"<0.05",round(pval[4],5))

names(m)
glm.dci(m,yID=5,xIDs=c(1:4),reverse=F,delta=0.001) -> rez

xlab1 <- "Province"
xlab2 <- "Gender & Age Group"
xlab3 <- "Location"
xlab4 <- "Reported Cause"
...
n3 <- length(unique(m$ghos))
n4 <- length(unique(m$DRgrp))

xCoord <- c((1:n1),(n1+2):(n1+n2+1),(n1+n2+3):(n1+n2+n3+2),(n1+n2+n3+3):(n1+n2+n3+n4+3))

pc1 <- rez[c(1:n1),3]
pc2 <- rez[(n1+1):(n1+n2),3]
pc3 <- rez[(n1+n2+1):(n1+n2+n3),3]
pc4 <- rez[(n1+n2+n3+1):(n1+n2+n3+n4),3]

yCoord <- 10*sqrt(c(pc1,pc2,pc3,pc4))
cilb1 <- rez[1:n1,4]
...
for (i in c(1:n4)) {
  points(xCoord[n1+n2+n3+1+i]+c(0,0),10*sqrt(c(cilb4[i],ciub4[i])),type="l",lwd=2)
}
...
# tree based method
m.rp <- rpart(tb~.,m,cp=0, minsplit=0)
printcp(m.rp)
...
m$gp2 <- predict(as.party(m.rp),type="node")	
addmargins(table(m$gp))

glm(family=binomial,data=m,tb~factor(gp2)) -> mod
summary(mod)
lroc(mod)$auc
